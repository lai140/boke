...
class Guide(PyroModule):
    ...
    def zc(self, x):
        with pyro.plate("data", x.size(0)):
            zc = pyro.sample("z", self.z_dist(x))
        return zc

    def zy(self, x):
        with pyro.plate("data", x.size(0)):
            zy = pyro.sample("z", self.zy_dist(x))
        return zy
